...
export const supabase = createClient(
  ...
  'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImJmbXN0ZGtudHBzZXl5aGlhcXphIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODE0MTgwNjAsImV4cCI6MjA5Njk5NDA2MH0.Eh57unrBX6uSRbQFj86oSHpgRv0ks41mS_ScHhM5F04'
)
...
export async function logRun({ snapshot, status, sources = [], rows = 0, error = null, ms = 0 }) {
  try {
    await supabase.from('fetch_log').insert({
      snapshot, status, sources,
      rows_written: rows,
      error_msg:   error,
      duration_ms: ms,
    })
...
export const err = (res, msg, code = 500) => res.status(code).json({ ok: false, error: msg })
