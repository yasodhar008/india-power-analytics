...
async function tryDownloadFirst(urlList, label) {
  for (const url of urlList) {
    try {
      ...
      console.log(
        `  ✓ ${label}: ${url} (${Math.round(buf.byteLength / 1024)} KB)`,
      );
...
async function updateICEDCapacity(year, month, figures) {
  ...
  if (figures.solar_gw)
    updates.push({
      fy_year: fy,
      fuel_type: "Solar",
      installed_gw: figures.solar_gw,
    });
  if (figures.wind_gw)
    updates.push({
      fy_year: fy,
      fuel_type: "Wind",
      installed_gw: figures.wind_gw,
    });
  if (figures.hydro_gw)
    updates.push({
      fy_year: fy,
      fuel_type: "Hydro",
      installed_gw: figures.hydro_gw,
    });
  if (figures.nuclear_gw)
    updates.push({
      fy_year: fy,
      fuel_type: "Nuclear",
      installed_gw: figures.nuclear_gw,
    });
...
export default async function handler(req, res) {
  ...
  await supabase.from("fetch_log").insert({
    ...
    status,
    sources: ["cea.nic.in/monthly-reports-archive"],
