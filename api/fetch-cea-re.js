...
async function findPDFFromListing(date) {
  try {
    ...
    while ((match = linkPattern.exec(html)) !== null) {
      const href = match[1];
      if (
        dateVariants.some((v) => href.includes(v)) ||
...
export default async function handler(req, res) {
  ...
  if (!pdfBuf) {
    await supabase.from("fetch_log").insert({
      snapshot: "cea-re-daily",
      status: "failed",
      sources: ["cea.nic.in/renewable-generation-report"],
      rows_written: 0,
      duration_ms: Date.now() - t0,
      error_msg:
        "PDF not found — check cea.nic.in/renewable-generation-report manually",
    });
  ...
  console.log(
    `  PDF downloaded: ${Math.round(pdfBuf.byteLength / 1024)} KB from ${pdfUrl}`,
  );
  ...
  return res.status(200).json({
    ...
    date,
    pdfUrl,
    sizeKB: Math.round(pdfBuf.byteLength / 1024),
    ...
    pythonUploaderNote:
      "Run: python upload_to_supabase.py --re path/to/cea_re.pdf --date " +
      date,
  });
}
