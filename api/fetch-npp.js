...
function extractFuelFigures(buf, reportNum) {
  ...
  const patterns = [
    { key: 'thermal_mu',  re: /(?:thermal|coal|lignite)[^\d\n]{0,60}([\d,]{4,10})/gi },
...
export default async function handler(req, res) {
  ...
  for (const report of REPORTS) {
    ...
    if (dl) {
      results[report.name] = {
        num: report.num, url: dl.url,
        fmt: dl.fmt, sizeKB: dl.sizeKB,
        desc: report.desc, priority: report.priority,
      }
  ...
  await supabase.from('fetch_log').insert({
    snapshot: 'npp-eod', status, sources: ['npp.gov.in'],
    rows_written: rowsWritten, duration_ms: ms,
    error_msg: nFound === 0 ? 'All NPP reports unavailable — published ~17:00-18:00 IST' : null,
  })

  return res.status(200).json({
    ok: true, date, status, reportsFound: nFound,
    reports: results, skipped, rowsWritten, ms,
