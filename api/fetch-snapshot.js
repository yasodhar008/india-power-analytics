...
const CEA_OPM_URL =
  "https://cea.nic.in/opm_grid_operation/daily-generation-report/?lang=en";
...
async function fetchCEAHourly(page, date, snapshot) {
  ...
  page.on("response", async (res) => {
    ...
    if (!ct.includes("json") && !ct.includes("csv") && !ct.includes("text"))
      return;
    try {
  ...
  try {
    ...
    const allUrls = [
      ...(captured.generation ? [captured.generation.url] : []),
      ...
      try {
        ...
        if (newUrl !== originalUrl) {
          ...
          if (data && data.length > 100) {
            if (originalUrl === captured.generation?.url)
              captured.generation.text = data;
            if (originalUrl === captured.demand?.url)
              captured.demand.text = data;
          }
        }
      } catch (_) {}
    }
...
async function fetchNPPFallback(date) {
  ...
  try {
    ...
    console.log(
      `  [NPP] dgr3 downloaded: ${(buf.byteLength / 1024).toFixed(0)} KB`,
    );
    return buf;
  } catch (e) {
...
function parseCEACSV(text, type, date, snapshot) {
  ...
  const srcIdx = headers.findIndex(
    (h) => h.includes("source") || h.includes("type") || h.includes("fuel"),
  );
  const valIdx = headers.findIndex(
    (h) => h.includes("value") || h.includes("mw") || h.includes("generation"),
  );
  const timeIdx = headers.findIndex(
    (h) => h.includes("time") || h.includes("date") || h.includes("hour"),
  );
  ...
  lines.slice(1).forEach((line) => {
    ...
    const hour = parseInt(
      (time.split(" ")[1] || time).split(":")[0] || "0",
      10,
    );
    const src =
      srcIdx >= 0
        ? (cols[srcIdx] || "").toUpperCase().trim()
        : type.toUpperCase();
...
function buildSummary(date, snapshot, genRows, demRows) {
  const get = (src) =>
    genRows.filter((r) => r.source?.includes(src)).map((r) => r.value_mw);
  const avg = (arr) =>
  ...
  const reShare = curDemand
    ? +(((curSolar + curWind) / curDemand) * 100).toFixed(1)
    : null;
  const notes = {
  ...
  return {
    ...
    cur_demand_mw: curDemand,
    cur_solar_mw: curSolar,
    cur_wind_mw: curWind,
    cur_thermal_mw: thermal.at(-1) ?? avg(thermal),
    re_share_pct: reShare,
    thermal_share_pct: curDemand
      ? +((avg(thermal) / curDemand) * 100).toFixed(1)
      : null,
...
async function saveAll(date, snapshot, genRows, demRows) {
  ...
  if (genRows.length || demRows.length) {
    await supabase
      ...
      .upsert(buildSummary(date, snapshot, genRows, demRows), {
        onConflict: "data_date,snapshot_time",
      });
    const solar = genRows
      .filter((r) => r.source?.includes("SOLAR"))
      .map((r) => r.value_mw);
    const wind = genRows
      .filter((r) => r.source?.includes("WIND"))
      .map((r) => r.value_mw);
    ...
    await supabase.from("power_daily_summary").upsert(
      {
        ...
        avg_re_share_pct: dem.length
          ? +(((avg(solar) + avg(wind)) / avg(dem)) * 100).toFixed(1)
          : null,
    ...
    if (!genRows.length) {
      ...
      if (buf) {
        ...
        await supabase.from("fetch_log").insert({
          snapshot,
          status: "partial",
          sources: ["npp-dgr3-fallback"],
          rows_written: 0,
          duration_ms: Date.now() - t0,
          error_msg:
            "CEA OPM returned no parseable data; dgr3 downloaded for manual review",
        });
