...
const SUPABASE_URL = import.meta.env.VITE_SUPABASE_URL || 'https://bfmstdkntpseyyhiaqza.supabase.co'
const SUPABASE_ANON_KEY = import.meta.env.VITE_SUPABASE_ANON_KEY || 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImJmbXN0ZGtudHBzZXl5aGlhcXphIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODE0MTgwNjAsImV4cCI6MjA5Njk5NDA2MH0.Eh57unrBX6uSRbQFj86oSHpgRv0ks41mS_ScHhM5F04'
...
export async function uploadDemandData(date, rows) {
  const records = rows.map(r => ({ data_date: date, hour: r.hour, value_mw: r.value_mw }))
  const { error } = await supabase
    .from('power_demand')
